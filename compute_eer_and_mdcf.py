...
##  ==========================================================================
##       author : Liang He
##   descrption : compute eer and mdcf
...
def compute_eer(fnr, fpr, st=''):
    ...
    return (fnr[x1] + a * ( fnr[x2] - fnr[x1] ))/3
...
def compute_mindcf_sre14(fnr, fpr,st=''):
...
def compute_eer_mdcf(scores, labels,st=''):
    
    [fnr, fpr] = compute_pmiss_pfa(scores, labels)
    eer = compute_eer(fnr, fpr,st)
    ...
    mindcf_sre14 = compute_mindcf_sre14(fnr, fpr,st)
...
def compute_eer_mdcff(scores, labels,st='new'):
    return compute_eer_mdcf(scores, labels,st)
def eval_ndx_score(fkey, flabel, fndx, fscore,st=''):
    ...
    [eer, mindcf_sre08, mindcf_sre10, mindcf_sre12, 
     mindcf_sre14, mindcf_sre16] = compute_eer_mdcf(scores, labels,st)
