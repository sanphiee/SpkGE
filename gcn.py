...
def sre14_load_data(save_data_path,device, thresh = 0.3):
    ...
    train_mask.astype(np.bool_)
    test_mask.astype(np.bool_)
    ...
    if os.path.exists(data_path):
        print("load data")
        edge_idx = torch.load(data_path, map_location = torch.device('cpu'))
...
class Net(nn.Module):
    def __init__(self,num_node_features,num_classes):
        ...
        self.conv_list.append(GCNConv(num_node_features, 256)) #
        for _ in range(2):
            # self.conv_list.append(GCNConv(256, 256))
            self.conv_list.append(GCNConv(256, 256))
            bn = BatchNorm1d(256, track_running_stats=False)
...
def main(thresh=0.3):
    ...
    for epoch in range(500):
        for data in loader:
            ...
            if (epoch+1) % 500 ==0:
                torch.save(gnn.state_dict(), save_data_path+f'gcn_{epoch}_{thresh}.ckpt')
...
def test(thresh=0.3):
    save_data_path ='./exp/'
    ckpt_path =save_data_path+f'gcn_499_{thresh}.ckpt'
